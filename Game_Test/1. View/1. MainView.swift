			...
			VStack
			{
				...
				Button(action:
					...
					if !gameEngine.isRunning
					{
						...
						// 플레이어가 죽어있을 경우, 살아있는 상태로 전환
						if gameEngine.player.isAlive == false
						{
							gameEngine.player.isAlive = true
							gameEngine.player.pos.x = Int(deviceWidth / 2)
							gameEngine.player.pos.y = Int(deviceHeight * 4 / 5)
							gameEngine.player.hitPoint = 100
						}
					}
